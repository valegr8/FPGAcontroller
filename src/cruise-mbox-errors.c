...
// ControlSems - used to create periodic behaviour
OS_EVENT *ControlSem;
OS_EVENT *VehicleSem;
OS_EVENT *ButtonSem;
OS_EVENT *SwitchSem;

// SW-Timer - to create periodic behaviour
OS_TMR *SwTmrControl;
OS_TMR *SwTmrVehicle;
OS_TMR *SwTmrIO;
...
/*
 * Check periodically whether the IO buttons have been pressed 
 * c/reates the signals CRUISE_CONTROL, GAS_PEDAL and BRAKE_PEDAL
 * lights a GREEN led to signal that the button has been activated.(LEDG2,LEDG4,LEDG6)
 */
void ButtonIOTask(void *pdata) 
{
  enum active cruise_control_flag = off;
  enum active gas_pedal_flag = off;
  enum active brake_pedal_flag = off;
  ...
  //get simbolic addresses
  volatile int * switches = (int *) DE2_PIO_TOGGLES18_BASE ;//D2_PIO_KEYS4_BASE; //pointer to the switched
  int * green_leds = (int *) DE2_PIO_GREENLED9_BASE; /* red_leds is a pointer to the LEDRs */

  //at the beginning we deactivate everything;
  *green_leds = 0;

  while(1)
  {
    //read buttons values
    int btn_val = buttons_pressed();
    //check if the buttons have been pressed
    //Cruise control
    if((btn_val & CRUISE_CONTROL_FLAG)== CRUISE_CONTROL_FLAG){
      *(green_leds) = *(green_leds) | LED_GREEN_2;
      if(!cruise_control_flag) {
        //activate
        *(green_leds) = *(green_leds) | LED_GREEN_0;
        if(DEBUG)
          printf("[ButtonIOTask] Cruise control activated - Buttons: %d, Leds: %d\n",btn_val, *green_leds);
        
        cruise_control_flag = on;
      } else {
        //deactivate
        *(green_leds) = *(green_leds) & ~(LED_GREEN_0);
        if(DEBUG)
          printf("[ButtonIOTask] Cruise control deactivated - Buttons: %d, Leds: %d\n",btn_val, *green_leds);
       
        cruise_control_flag = off;
      }
    } else {
        *(green_leds) = *(green_leds) & ~(LED_GREEN_2);
    }
    //Gas pedal
    if((btn_val & GAS_PEDAL_FLAG)== GAS_PEDAL_FLAG){
        *(green_leds) = *(green_leds) | LED_GREEN_6;
        if(DEBUG)
          printf("[ButtonIOTask] Gas pedal active - Buttons: %d, Leds: %d\n",btn_val, *green_leds);
        
        /*if(!gas_pedal_flag) {
        //activate
        *(green_leds) = *(green_leds) | LED_GREEN_2;
        printf("[ButtonIOTask] Gas pedal activated - Buttons: %d, Leds: %d\n",btn_val, *green_leds);
        gas_pedal_flag = 1;
      } else {
        //deactivate
        *(green_leds) = *(green_leds) & ~(LED_GREEN_2);
        printf("[ButtonIOTask] Gas pedal deactivated - Buttons: %d, Leds: %d\n",btn_val, *green_leds);
        gas_pedal_flag = 0;
      }*/
    }else {
        *(green_leds) = *(green_leds) & ~(LED_GREEN_6);
    ...
    //Brake pedal
    if((btn_val & BRAKE_PEDAL_FLAG)== BRAKE_PEDAL_FLAG){
        //brake on
        *(green_leds) = *(green_leds) | LED_GREEN_4;
        brake_pedal_flag = on;
        
        OSMboxPost(Mbox_Brake, (void *)1);
        if(DEBUG)
          printf("[ButtonIOTask] Brake pedal active - Buttons: %d, Leds: %d\n",btn_val, *green_leds);
   }else {
        //brake off
        brake_pedal_flag = off;
        if(DEBUG)
          printf("[ButtonIOTask] Brake pedal NOT active - brake_flag %d\n", brake_pedal_flag);
          
        //OSMboxPost(Mbox_Brake, (void *)&brake_pedal_flag);
        *(green_leds) = *(green_leds) & ~(LED_GREEN_4);
    } 
...
{ 
  ...
  if(DEBUG)
    printf("[VehicleTask] Initial brake val: %d\n", brake_pedal);
  ...
  while(1)
  {
    ...
    if (err == OS_NO_ERR){ 
      ...
      if(DEBUG)
        printf("[VehicleTask] Brake: %d\n", brake_pedal);
      
    }else {
      printf("[VehicleTask] Brake Mbox Error!\n");
    }
...
void ControlTask(void* pdata)
{
  ...
  while(1)
  {
    ...
    //Wait semaphore
    //printf("[ControlTask] Waiting for %d\n", OSTmrRemainGet(SwTmrControl,&err));
    OSSemPend(ControlSem, 0, &err); 
    //The semahopre is then signaled by the SWTimer
 ...
 */
void ControlTmrCallback(OS_TMR *ptmr, void *p_arg)
{
  INT8U err = OSSemPost(ControlSem);
  //printf("[ControlTmrCallback] Signal to control task\n");
}


void VehicleTmrCallback(OS_TMR *ptmr, void *p_arg)
{

  INT8U err = OSSemPost(VehicleSem);
  //printf("[VehicleTmrCallback] Signal to vehicle task\n");
}

void IOTmrCallback(){
  //printf("[IOTmrCallback] Callback\n");
  INT8U err = OSSemPost(ButtonSem);
  err = OSSemPost(SwitchSem);
}
...
void StartTask(void* pdata)
{
   ...
   */
   SwTmrControl = OSTmrCreate(0,                            /* Initial delay */
                          CONTROL_PERIOD / HW_TIMER_PERIOD,                   /* Repeat period */
                          OS_TMR_OPT_PERIODIC,              /* Options - periodic timer */
                          (OS_TMR_CALLBACK)ControlTmrCallback,   /* Function to call when the timer reaches 0 */
                          NULL,                             /* Arg. for the callback */
                          (INT8U *)"ControlSWTimer",               /* Name of timer, ASCII */
                          (INT8U *)&err);
  

    /* Check 'err' */
    OSTmrStart(SwTmrControl, &err);

    if (err == OS_ERR_NONE) {
      if(DEBUG)
        printf("[StartTask] Control SW Timer started\n");
    }else {
      printf("[StartTask] Timer error in the start of Control SW Timer\n");
      //exit(1);
    } 

   SwTmrVehicle = OSTmrCreate(0,                            /* Initial delay */
                          VEHICLE_PERIOD / HW_TIMER_PERIOD,                   /* Repeat period */
                          OS_TMR_OPT_PERIODIC,              /* Options - periodic timer */
                          (OS_TMR_CALLBACK)VehicleTmrCallback,   /* Function to call when the timer reaches 0 */
                          NULL,                             /* Arg. for the callback */
                          (INT8U *)"VehicleSWTimer",               /* Name of timer, ASCII */
                          (INT8U *)&err);
  
   SwTmrIO = OSTmrCreate(0,                            /* Initial delay */
                          IO_PERIOD / HW_TIMER_PERIOD,                   /* Repeat period */
                          OS_TMR_OPT_PERIODIC,              /* Options - periodic timer */
                          (OS_TMR_CALLBACK)IOTmrCallback,   /* Function to call when the timer reaches 0 */
                          NULL,                             /* Arg. for the callback */
                          (INT8U *)"IOSWTimer",               /* Name of timer, ASCII */
                          (INT8U *)&err);

    /* Check 'err' */
    OSTmrStart(SwTmrVehicle, &err);
    if (err == OS_ERR_NONE) {
      if(DEBUG)
        printf("[StartTask] Vehicle SW Timer started\n");
    }else {
      printf("[StartTask] Timer error in the start of Vehicle SW Timer\n");
      //exit(1);
    } 


    OSTmrStart(SwTmrIO, &err);
    if (err == OS_ERR_NONE) {
      if(DEBUG)
        printf("[StartTask] IO SW Timer started\n");
    }else {
      printf("[StartTask] Timer error in the start of IO SW Timer\n");
      //exit(1);
    } 
  /*
  ...
  //ControlSems
  ControlSem = OSSemCreate(0); /* Semaphore - Initialized to 0 */
  VehicleSem = OSSemCreate(0); /* Semaphore - Initialized to 0 */
  ButtonSem = OSSemCreate(0); /* Semaphore - Initialized to 0 */
  SwitchSem = OSSemCreate(0); /* Semaphore - Initialized to 0 */
